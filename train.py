...
x2 = tf.placeholder(tf.float32, shape=[64,64,3])
y2_ = tf.placeholder(tf.float32, shape=[30,3])

x = tf.placeholder(tf.float32, shape=[30, 64, 64, 3])
y_ = tf.placeholder(tf.float32, shape=[30, 3])

W_conv1 = weight_variable([5, 5, 3, 64])
b_conv1 = bias_variable([64])

# x_image = tf.reshape(x, [-1,64,32,1])
...
if sys.argv[1] == "train":
	...
	for x in range(1,46):
		path = baseAtypicalPath + str(x) + "/picture.bmp"
		dataAtypical.extend(misc.imread(path))

		path2 = baseMelonomaPath + str(x) + "/picture.bmp"
		dataMelonoma.extend(misc.imread(path2))

		path3 = baseNevusPath + str(x) + "/picture.bmp"
		dataNevus.extend(misc.imread(path3))
